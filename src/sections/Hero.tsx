...
export default function Hero() {
  ...
  useEffect(() => {
    const sections = ["method", "features", "decks", "progress", "srs", "download"];
  ...
  // Helper para las clases
  const navLinkClass = (section: string) =>
    activeSection === section
      ? "text-primary font-bold border-b-2 border-primary pb-1 text-sm"
      : "text-on-surface-variant hover:text-primary transition-colors text-sm font-semibold border-b-2 border-transparent pb-1";
  return (
    <>
      {/* Navigation */}
      <nav className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-md shadow-sm">
        <div className="flex items-center h-20 px-6 max-w-7xl mx-auto gap-4">
          ...
          <div className="hidden md:flex flex-1 justify-center gap-8 items-center">
            <a className={navLinkClass("method")} href="#method">{t.nav.method}</a>
            <a className={navLinkClass("features")} href="#features">{t.nav.features}</a>
            <a className={navLinkClass("decks")} href="#decks">{t.nav.decks}</a>
            <a className={navLinkClass("progress")} href="#progress">{t.nav.progress}</a>
            <a className={navLinkClass("srs")} href="#srs">{t.nav.srs}</a>
            <a className={navLinkClass("download")} href="#download">{t.nav.download}</a>
          </div>
      ...
      {/* Hero Section */}
      <header className="pt-32 pb-20 md:pt-48 md:pb-32 bg-gradient-to-br from-[#f8fafa] to-[#e3fffe] overflow-hidden" id="method">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <div className="flex flex-wrap gap-4">
              <a className="bg-primary-container text-on-primary-container px-8 py-4 rounded-xl font-semibold inline-flex items-center gap-2 hover:shadow-lg transition-all" href="#download">
                <Play size={20} fill="currentColor" />
                {t.cta.downloadPlay}
              </a>
              <a className="border-2 border-primary text-primary px-8 py-4 rounded-xl font-semibold inline-flex items-center gap-2 hover:bg-primary/5 transition-colors" href="#features">
                {t.cta.learnMore}
                <ArrowRight size={20} />
              </a>
            </div>
