...
export default function Features() {
    const [locale, setLocale] = useState<"es" | "en" | "pt">(multiLocaleInitializer);

    function multiLocaleInitializer(): "es" | "en" | "pt" {
        if (typeof window !== "undefined") {
            const saved = window.localStorage.getItem("recallnest_locale");
            if (saved === "en" || saved === "es" || saved === "pt") {
                return saved as "es" | "en" | "pt";
            }
        }
        return "es"; 
    }
    ...
    return (
        <section id="features" className="py-24 bg-surface overflow-hidden">
            <div className="max-w-7xl mx-auto px-6">
                ...
                >
                    {/* Card 1 */}
                    <motion.div variants={itemVariants} className="bg-white p-8 rounded-2xl shadow-sm border border-outline-variant/30 flex flex-col items-center text-center hover:shadow-md transition-shadow">
                    ...
                    {/* Card 2 - Destacada */}
                    <motion.div variants={itemVariants} className="bg-white p-8 rounded-2xl shadow-xl border-2 border-primary/10 flex flex-col items-center text-center scale-105 relative z-10">
                ...
                >
                    {/* Modo Normal */}
                    <motion.div variants={itemVariants} className="bg-[#f2f4f4] p-6 rounded-2xl flex flex-col gap-4">
